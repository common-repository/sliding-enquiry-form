...
class Sliding_Enquiry {
	...
	public function __construct() {
		...
		$this->enquiry_discription = get_option( 'sp_enquiry_discription' );
	...
	public function load_sliding_enquiry () {
		...
		if($this->enquiry_active)
		{
			...
			if($show_sliding_enquiry)
			{
				...
				//add_action( 'wp_enqueue_scripts', array( $this, 'font_awesome_style' ) );
	...
	public function sliding_enquiry_options() {
		...
		if ( ! empty( $_POST ) && check_admin_referer( 'sliding_enquiry', 'save_sliding_enquiry' ) ) {
			...
			//add or update Sliding Enquiry title options
			if ( $this->enquiry_discription !== false ) {
				update_option( 'sp_enquiry_discription', $_POST['enquiry_discription'] );
			} else {
				add_option( 'sp_enquiry_discription', $_POST['enquiry_discription'], null, 'no' );
			}			
		...
		<div class="wrap">
			...
			<p>Mail Functionality working only live website. please make sure your website uploaded on live server??</p>
			<form method="post" action="<?php echo esc_url( admin_url( 'options-general.php?page='.$_GET['page'].'&noheader=true' ) ); ?>" enctype="multipart/form-data">
				...
				<div class="sticky_popup_form">
					<table class="form-table" width="100%">
						...
						<tr>
							<th scope="row"><label for="enquiry_discription"><?php _e( 'Enquiry Discription', 'sliding-enquiry' );?></label></th>
							<td><textarea name="enquiry_discription" id="enquiry_discription" maxlength="255" size="25"><?php echo $this->enquiry_discription; ?></textarea></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><input type="text" name="enquiry_title_image" id="enquiry_title_image" maxlength="255" size="25" value="<?php echo $this->enquiry_title_image; ?>"></td>
						</tr>
						<tr>
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( "font-awesome-min", plugins_url( 'css/font-awesome.min.css', __FILE__ ), array(), self::VERSION );
	}

	public function font_awesome_style() {
		wp_enqueue_style( $this->plugin_slug . '-style', plugins_url( 'css/font-awesome.min.css', __FILE__ ), array(), self::VERSION );
	}
	...
	*/ 
	...
	public function get_sliding_enquiry(){
		...
		$this->enquiry_place  = get_option( 'sp_enquiry_place' );
		$this->enquiry_title  = get_option( 'sp_enquiry_title' );
		$this->sp_enquiry_discription  = get_option( 'sp_enquiry_discription' );
		$this->enquiry_title_color  = get_option( 'sp_enquiry_title_color' );
		$this->enquiry_title_image = get_option( 'sp_enquiry_title_image' );
		$this->enquiry_header_color = get_option( 'sp_enquiry_header_color' );
		$this->enquiry_header_border_color = get_option( 'sp_enquiry_header_border_color' );
		...
		if($this->enquiry_title_image != ''){
			$enquiryTitleImage = $this->enquiry_title_image;
		} else {
			$enquiryTitleImage = "fa fa-angle-double-down";
		}
		if($this->enquiry_title != ''){
			$enquiryTitle = $this->enquiry_title;
		} else {
			$enquiryTitle = "Enquiry Now";
		}
		...
		if($this->enquiry_place!='top-left' && $this->enquiry_place!='top-right')
		{
			...
			if( $this->enquiry_title_image != '') {
				...
				$enquiry_html .= '<i class="'.$enquiryTitleImage.'"></i>';	
	...
	var pattern = /^\d{10}$/;
		...
		if($this->enquiry_discription != ''){
			$enquiryDescription = $this->enquiry_discription;
		} else {
			$enquiryDescription = "Please fill up the form below and our advisors will get in touch you.";
		}
		...
		//if( $this->enquiry_content != '') {
				...
				$enquiry_html .= '<div>';
				$enquiry_html .= '<p class="equirydescription">'.$enquiryDescription.'</p>';
				$enquiry_html .= '<hr/>';
				...
				$enquiry_html .= '</li>';
				$enquiry_html .= '<li>';
				$enquiry_html .= '<input type="text" name="number" minlength="10"  id="number" placeholder="Mobile No." class="textbox_popup" value="">';
				$enquiry_html .= '</li>';
				$enquiry_html .= '<li>';
				$enquiry_html .= '<textarea name="message" id="message"  placeholder="Message" class="textbox_popup textareacustom" value=""></textarea>';
				$enquiry_html .= '</li>';
				$enquiry_html .= '<li class="slidingcaptcha">';
				...
				$enquiry_html .= '</li>';
				$enquiry_html .= '<li class="slidingcaptchaans">';
				$enquiry_html .= '<input type="text" name="captcha"  id="captcha" value="" Placeholder="Captcha" class="textbox_popup"/>';
				$enquiry_html .= '</li>';
				$enquiry_html .= '<li>';
				$enquiry_html .= '<input type="button" name="submit" id="submit" class="button1_submit buttoncustomsubmit" style="" value="Enquiry Now"/>';
				$enquiry_html .= '</li>';
				$enquiry_html .= '</ul>';
				$enquiry_html .= '</form>';
				$enquiry_html .= '<hr/>';
